...
def read_img(image_name: str, target_h: int = 1000) -> Image:
    ...
    img = Image.open(os.path.join(arg.image_path,image_name))
    img = ImageOps.exif_transpose(img)  # 이미지 정보에 따라 이미지를 회전

    ann_dict = get_ann(os.path.join(arg.image_path,image_name),arg.api_url)
...
def initial_set():
    ...
    img_idx = 0
    img_list = os.listdir(arg.image_path)
    ...
    if len(img_list)%(arg.row*arg.column) != 0:
        max_idx = len(img_list)//(arg.row*arg.column)
    else: 
        max_idx = len(img_list)//(arg.row*arg.column)-1
...
if __name__ == "__main__":
    ...
    # parser.add_argument('-i','--image_path',default='/opt/ml/upstage_OCR/Data set/real data/general')
    parser.add_argument('-i','--image_path',default='/opt/ml/upstage_OCR/Data set/real data/receipt')
    # parser.add_argument('-i','--image_path',default='/opt/ml/upstage_OCR/Data set/unity data')
    ...
    parser.add_argument('-u','--api_url',default="http://118.222.179.32:30000/ocr/")
